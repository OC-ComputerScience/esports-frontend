...
const actions = [
  ...
  { label: "Versions", event: "view-versions" },
...
const handleActionEvent = (payload) => {
  ...
  if (payload.event == "view-versions") {
    router.push({
      name: "maintianFormVersions",
      params: { formId: payload.value },
    });
  }
...
const getForms = () => {
  FormServices.getAllForms(itemsPerPage.value, page.value)
    .then((response) => {
...
const search = (filter) => {
  if (filter == "" || filter == null) {
    getForms(itemsPerPage.value, page.value);
  } else {
